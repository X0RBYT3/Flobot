...
def objdiff(obj1, obj2, *, access_attr=None, depth=0):
    ...
    if access_attr is None:
        def attrdir(x): return x
    elif access_attr == 'auto':
        if hasattr(obj1, '__slots__') and hasattr(obj2, '__slots__'):
            def attrdir(x): return getattr(x, '__slots__')

        elif hasattr(obj1, '__dict__') and hasattr(obj2, '__dict__'):
            def attrdir(x): return getattr(x, '__dict__')
        else:
            attrdir = dir
    elif isinstance(access_attr, str):
        def attrdir(x): return list(getattr(x, access_attr))
    else:
        attrdir = dir
    for item in set(attrdir(obj1) + attrdir(obj2)):
        try:
            ...
            log.warning(
                "Error checking {o1}/{o2}.{item}".format(o1=obj1, o2=obj2, item=item), exc_info=e)
            continue
    return changes
